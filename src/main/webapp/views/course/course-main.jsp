        ...
        <body>
                        ...
                        <form id="autoScheduleForm" onsubmit="submitAutoSchedule(event)">
                            <div class="form-row">
                                <div class="form-group">
                                    ...
                                    <input type="date" id="autoStartDate" name="start_date" class="form-control"
                                        required>
                                </div>
                                <div class="form-group">
                            ...
                            <div class="form-group">
                                ...
                                <div class="checkbox-group form-control" id="courseCheckboxGroup"
                                    style="height: 160px;">
                                    <!-- 課程選項會動態載入這裡 -->
                                </div>
                            ...
                            <div class="form-group">
                                ...
                                <div class="checkbox-group form-control" id="subjectCheckboxGroup"
                                    style="height: 160px;">
                                    <!-- 科目選項會動態載入這裡 -->
                                </div>
                                <div class="selected-display">
                            ...
                            <div class="form-group">
                                ...
                                <div class="checkbox-group form-control" id="teacherCheckboxGroup"
                                    style="height: 160px;">
                                    <!-- 老師選項會動態載入這裡 -->
                                </div>
                                <div class="selected-display">
                            ...
                            <div class="form-group">
                                ...
                                <div class="checkbox-group form-control" id="classroomCheckboxGroup"
                                    style="height: 90px;">
                                    <!-- 教室選項會動態載入這裡 -->
                                </div>
                                <div class="selected-display">
                            ...
                            <div class="form-row">
                                <div class="form-group">
                                    ...
                                    <input type="number" id="maxCapacity" name="max_capacity" class="form-control"
                                        min="1" placeholder="最多幾人">
                                </div>
                                <div class="form-group">
                                    <label class="form-label" for="minCapacity">最少人數</label>
                                    <input type="number" id="minCapacity" name="min_capacity" class="form-control"
                                        min="0" placeholder="最少幾人開班">
                                </div>
                            </div>
                            <div class="form-group">
                        ...
                        <form id="manualScheduleForm">
                            ...
                            <div class="form-row">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="number" id="periodStart" name="periodStart" class="form-control"
            ...
            <script>
                ...
                async function loadOptions() {
                    const courseRes = await fetch('${pageContext.request.contextPath}/api/course/all').then(r => r.json());
                ...
                async function updateStats(courses) {
                    ...
                    // 總學生數
                    const res = await fetch('${pageContext.request.contextPath}/api/course/enroll/studentCount').then(r => r.json());
                    ...
                    try {
                        ...
                        document.getElementById('totalStudents').textContent = '-';
                        console.error('[updateStats] 學生數 API 錯誤:', error);
                        return;
            ...
            <tr>
                ...
                async function saveCourse(event) {
                    ...
                    const course = {
                        ...
                        minCapacity: minCapacity && minCapacity.trim() !== '' ? parseInt(minCapacity) : null,
                        fee: fee && fee.trim() !== '' ? parseInt(fee) : null,
                    ...
                    if (!course.name || !course.type || !course.maxCapacity || !course.startDate || !course.endDate) {
                        Swal.fire({
                            icon: 'warning',
                            title: '請填寫所有必填欄位',
                            text: '課程名稱、類型、最大人數、開始日期、結束日期為必填項目',
                            confirmButtonText: '確定'
                        });
                        return;
                    }
                    ...
                    const exists = await fetch('${pageContext.request.contextPath}/api/course/checkExists?courseId=' + id, 
                         {
                             method: 'GET',
                             headers: {
                      ...
                      const existsResult = await exists.text();

                      if(existsResult ==='exists'){
                        fetch('${pageContext.request.contextPath}/api/course/update', {
                        method: 'PUT',
                        headers: {
                            'Content-Type': 'application/json; charset=UTF-8',
                ...
                async function deleteCourse(id) {
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            fetch('${pageContext.request.contextPath}/api/course/delete?id=' + id, { method: 'DELETE' })
                ...
                document.addEventListener('DOMContentLoaded', () => {
                    ...
                    document.getElementById('manualScheduleForm').addEventListener('submit', submitManualSchedule);
                });

                // 手動排課表單提交
                async function submitManualSchedule(event) {
                    ...
                    const scheduleData = {
                        ...
                        lessonDate: lessonDate && lessonDate.trim() !== '' ? lessonDate : null,
                    ...
                    console.log("手動排課資料：", scheduleData);

                    // 簡單驗證
                    if (!scheduleData.courseId || !scheduleData.subjectId || !scheduleData.teacherId ||
                        !scheduleData.classroomId || !scheduleData.lessonDate ||
                        !scheduleData.periodStart || !scheduleData.periodEnd) {
                        Swal.fire({
                            icon: 'warning',
                            title: '請填寫所有必填欄位',
                            text: '所有欄位都是必填項目',
                    ...
                    try {
                        const response = await fetch('${pageContext.request.contextPath}/api/schedule/add', {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json; charset=UTF-8'
                            },
                        ...
                        console.log('手動排課回傳：', message);

                        if (response.ok) {
                            const isSuccess = message.includes('成功更新課表') || message.includes('成功');

                            Swal.fire({
                                icon: isSuccess ? 'success' : 'error',
                                title: isSuccess ? '手動排課成功！' : '手動排課失敗',
                                text: message,
                                confirmButtonText: '確定'
                            }).then(() => {
                                if (isSuccess) {
